...
class CrawlReward:
    """HumanoidBench-style Crawl reward with batched ``TensorState``."""

    def __init__(self, robot_name: str = "h1"):
        self.robot_name = robot_name
        quat_raw = torch.tensor([0.75, 0.0, 0.65, 0.0])
        self._crawl_height = 0.8
...
@register_task("humanoid.crawl", "crawl", "h1.crawl")
class CrawlEnv(BaseLocomotionEnv):
    ...
    def __init__(self, scenario: ScenarioCfg, device: str | torch.device | None = None) -> None:
        ...
        self.reward_functions = [CrawlReward(self.robot_name)]
