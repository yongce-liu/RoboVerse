...
from roboverse_learn.rl.unitree_rl.configs.locomotion.g1_dof29_walking import G1Dof29WalkingCfg
...
class G1Dof29WalkingTask(Humanoid):
    ...
    task_config = G1Dof29WalkingCfg(robots=[get_robot("g1_dof29")])
    scenario = ScenarioCfg(
        robots=[get_robot("g1_dof29")],
    ...
    def __init__(self, scenario: ScenarioCfg, device: str | torch.device | None = None):
        ...
        task_cfg = G1Dof29WalkingCfg(robots=[get_robot("g1_dof29")])
    ...
    def _compute_ref_state(self, envstate):
        ...
        self.ref_dof_pos = torch.zeros(self.num_envs, self.robot.num_joints, device=self.device, requires_grad=False)
