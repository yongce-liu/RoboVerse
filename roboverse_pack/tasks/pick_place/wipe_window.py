...
class WipeWindow(RLTaskEnv):
    ...
    def _prepare_states(self, states, env_ids):
        ...
        # Keep window in place (no randomization)
        window_pos = states.objects["window"].root_state[:, 0:3].clone()
        window_quat = states.objects["window"].root_state[:, 3:7].clone()
        states.objects["window"].root_state = torch.cat([window_pos, window_quat, zero_vel, zero_ang_vel], dim=-1)
