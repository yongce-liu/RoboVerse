...
class PickPlaceObstacle(RLTaskEnv):
    ...
    def _prepare_states(self, states, env_ids):
        ...
        # Keep wall in place (no randomization)
        wall_pos = states.objects["wall"].root_state[:, 0:3].clone()
        wall_quat = states.objects["wall"].root_state[:, 3:7].clone()
        states.objects["wall"].root_state = torch.cat([wall_pos, wall_quat, zero_vel, zero_ang_vel], dim=-1)
    ...
    def _reward_gripper_approach(self, env_states) -> torch.Tensor:
        ...
        approach_reward = 1 - torch.tanh(gripper_box_dist)
