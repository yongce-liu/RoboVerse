...
class Controller:
    ...
    def run(self):
        ...
        is_dex3 = num_actions > 29  # hand joints present
        if is_dex3:
            # dex3 observation layout
            self.obs[:3] = self.cmd * self.config.cmd_scale
            self.obs[3:6] = ang_vel
            self.obs[6:9] = base_euler_xyz
            self.obs[9:12] = gravity_orientation
            ...
            self.obs[12 + num_actions * 3] = sin_phase
            self.obs[12 + num_actions * 3 + 1] = cos_phase
        else:
            self.obs[:3] = ang_vel
...
if __name__ == "__main__":
    ...
    # # Load config
    config_path = f"roboverse_learn/unitree_rl/deploy/configs/{args.config}"
